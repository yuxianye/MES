...
{
    public class EquipmentTypeInfoViewModel : PageableViewModelBase
    {
        public EquipmentTypeInfoViewModel()
        {
            //this.MenuModule
            initCommand();
            registerMessenger();
        ...
        /// 设备类别信息模型
        /// </summary>
        private EquipmentTypeModel equipmentTypeInfo;// = new EnterpriseModel();
        /// <summary>
        /// 设备类别信息模型
        /// </summary>
        public EquipmentTypeModel EquipmentTypeInfo
        {
            get { return equipmentTypeInfo; }
            set { Set(ref equipmentTypeInfo, value); }
        }
        #endregion
        ...
        {
            Messenger.Default.Register<EquipmentTypeModel>(this, MessengerToken.DataChanged, dataChanged);
        }
        ...
        {
            Messenger.Default.Unregister<EquipmentTypeModel>(MessengerToken.DataChanged, dataChanged);
        }
        #endregion
        ...
        private void OnExecuteAddCommand()
        {
            ...
            var menuFunctionViewInfoMap = GlobalData.MenuFunctionViewInfoMap.FirstOrDefault(a => a.Controller == "EquipmentTypeInfo" && a.Action == "Add");
        ...
        private bool OnCanExecuteAddCommand()
        {
            ...
            {
                //与服务端对应
                if (base.MenuModule.Functions.Any(a => a.Controller == "EquipmentTypeInfo" && a.Action == "Add"))
                {
                    return true;
                }
                else
        ...
        private void OnExecuteEditCommand()
        {
            ...
            var menuFunctionViewInfoMap = GlobalData.MenuFunctionViewInfoMap.FirstOrDefault(a => a.Controller == "EquipmentTypeInfo" && a.Action == "Update");
            var viewInfo = menuFunctionViewInfoMap?.ViewInfo;
            viewInfo.Parameter = this.EquipmentTypeInfo.Clone();
        ...
        private bool OnCanExecuteEditCommand()
        {
            ...
            {
                if (base.MenuModule.Functions.Any(a => a.Controller == "EquipmentTypeInfo" && a.Action == "Update"))
                {
        ...
        private async void OnExecutePhysicalDeleteCommand()
        {
            var window = (MetroWindow)Application.Current.MainWindow;
            var dialogResult = await window.ShowMessageAsync("数据删除"
                , string.Format("确定要从磁盘中删除【{0}】这条数据么？删除后无法恢复。" + System.Environment.NewLine + "确认删除点击【是】，取消删除点击【否】。", EquipmentTypeInfo.EquipmentTypeName)
                , MessageDialogStyle.AffirmativeAndNegative, new MetroDialogSettings() { AffirmativeButtonText = "是", NegativeButtonText = "否" });
            if (dialogResult == MessageDialogResult.Affirmative)
            {
                var result = Utility.Http.HttpClientHelper.PostResponse<OperationResult>(GlobalData.ServerRootUri + "EquipmentTypeInfo/Remove", Utility.JsonHelper.ToJson(new List<Guid>() { EquipmentTypeInfo.Id }));
                if (!Equals(result, null) && result.Successed)
                {
        ...
        private bool OnCanExecutePhysicalDeleteCommand()
        {
            ...
            {
                if (base.MenuModule.Functions.Any(a => a.Controller == "EquipmentTypeInfo" && a.Action == "Remove"))
                {
                    if (Equals(EquipmentTypeInfo, null))
                    {
                        return false;
        ...
        private void OnExecuteSearchCommand(string txt)
        {
            ...
            FilterRule filterRuleName = new FilterRule("EquipmentTypeName", txt.Trim(), FilterOperate.Contains);
            FilterRule filterRuleCode = new FilterRule("EquipmentTypeCode", txt.Trim(), FilterOperate.Contains);
